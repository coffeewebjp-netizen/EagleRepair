...
- 外部ディスプレイ復旧
  - アクティブ画面数とDell SE2425HGのPnP接続を診断
  - Windowsの「拡張」を再要求
  - 「外部画面のみ」から「拡張」へ切り替えてNVIDIAの外部出力を再初期化
  - 必要な場合だけ追加のPnP再スキャンと表示構成の再作成
  - 3画面とSE2425HGの接続を確認してから成功判定
...
- 外部ディスプレイ復旧では画面が数秒点滅する場合があります。GPUの無効化・有効化、ドライバー再起動、更新は行いません。
...
- Eagle、外部ディスプレイ復旧、DDPMの3モジュールが自動表示されることを確認
...
- 欠落時にアクティブ2画面、SE2425HG未接続、`TARGET_MONITOR_MISSING_AFTER_WAKE`を検出
- 復帰後にアクティブ3画面、SE2425HG接続、`HEALTHY`を検出
- 正常状態の修復は通常権限で変更を加えずSuccess=true、ExitCode=0で完了
...
- [外部ディスプレイ復帰障害記録](display-wake-incident-20260717.md)
